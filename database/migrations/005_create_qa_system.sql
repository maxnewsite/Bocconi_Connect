...
-- Indexes
CREATE INDEX idx_questions_asker ON qa_questions(asker_id);
CREATE INDEX idx_questions_category ON qa_questions(category);
CREATE INDEX idx_questions_created ON qa_questions(created_at DESC);
CREATE INDEX idx_questions_tags ON qa_questions USING GIN(tags);
CREATE INDEX idx_questions_unanswered ON qa_questions(has_accepted_answer) WHERE has_accepted_answer = false;
CREATE INDEX idx_answers_question ON qa_answers(question_id);
CREATE INDEX idx_answers_answerer ON qa_answers(answerer_id);
CREATE INDEX idx_answers_accepted ON qa_answers(is_accepted) WHERE is_accepted = true;
CREATE INDEX idx_votes_user ON qa_votes(user_id);
CREATE INDEX idx_votes_votable ON qa_votes(votable_type, votable_id);
...
CREATE POLICY "Askers can update own questions"
...
CREATE POLICY "Answerers update own answers"
ON qa_answers FOR UPDATE
USING (auth.uid() = answerer_id);

CREATE POLICY "Question askers can accept answers"
ON qa_answers FOR UPDATE
USING (
...
ON qa_votes FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can view all votes"
ON qa_votes FOR SELECT
USING (true);

CREATE POLICY "Users can delete own votes"
ON qa_votes FOR DELETE
...
-- Comments
COMMENT ON TABLE qa_questions IS 'Community questions for knowledge sharing';
COMMENT ON TABLE qa_answers IS 'Answers to community questions';
COMMENT ON TABLE qa_votes IS 'Upvotes and downvotes for questions and answers';
COMMENT ON TABLE user_reputation IS 'User reputation scores and badges based on Q&A activity';
